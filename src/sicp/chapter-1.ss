...
;; Process generated by `(count-change 11) => 4`:
;;
;; ```
;; (cc 11 5)
;;   (cc -39 5) => 0
...
;; (b) During the process, `p` is evaluated $n$ times such that
;; $\theta/3^n\lt0.1$. Solving for $n$ gives us $n = \log(10\theta)/\log(3)$,
;; thus the number of steps for `sine` grows as $\Theta(\log n)$. The
;; interpreter must maintain the stack for that number of calls to `p`,
;; therefore the space complexity is also $\Theta(\log n)$.
...
;; Let Tpq tranform (a, b) to (bq + aq + ap, bp + aq).
;; Then Tpq applied twice to (a, b) gives:
;;     Tpq(Tpq(a, b))
;;   = Tpq(bq + aq + ap, bp + aq)
